...
class PenyuluhPNSKecModel extends Model
{
    ...
    {
        $query = $this->db->query("select *, a.nama, a.nip, a.agama, a.keahlian, a.email, d.nm_desa,
        case a.kode_kab 
                                when '3' then 
                                    case a.unit_kerja 
    ...
    public function getPenyuluhPNSKecTotal($kode_kec)
    {
        ...
        where a.tempat_tugas='" . $kode_kec . "' and status not IN('1','2')");
        $row   = $query->getRow();
        ...
        $query   = $db->query("select a.nip_lama, a.nip, a.nama, a.gelar_dpn, a.gelar_blk, a.tgl_update, k.nama_bpp,
                                ...
                                left join tbldaerah j on a.kecamatan_tugas=j.id_daerah
                                where a.tempat_tugas='" . $kode_kec . "' and status not IN('1','2') ORDER BY nama");
